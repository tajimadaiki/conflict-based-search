...
class Constraints:
    ...
    def __init__(self):
        self.constraints_on_node: Dict[Agent, Dict[int, Set[Tuple[int, int]]]] = dict()
        self.constraints_on_edge: Dict[Agent, Dict[int, Dict[Tuple[int, int], Set[Tuple[int, int]]]]] = dict()

    def fork_node_conflict(self, agent: Agent, time: int, obstacle: Tuple[int, int]) -> 'Constraints':
        # copy constrains and insert constrain
        constraints_on_node_copy = deepcopy(self.constraints_on_node)
        constraints_on_edge_copy = deepcopy(self.constraints_on_edge)
        constraints_on_node_copy.setdefault(agent, dict()).setdefault(time, set()).add(obstacle)
        # create new constrain instance
        new_constraints = Constraints()
        new_constraints.constraints_on_node = constraints_on_node_copy
    ...
    def __str__(self):
        return "node_conf:" + str(self.constraints_on_node) + \
...
if __name__ == "__main__":
    ...
    constraints_3 = constraints_2.fork_node_conflict(agent_2, 20, (6, 4))
    constraints_4 = constraints_3.fork_edge_conflict(agent_1, 30, (3, 4), (4, 4))

    print(constraints_4.constraints_on_node[agent_1], constraints_4.constraints_on_edge[agent_1])
